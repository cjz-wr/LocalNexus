...
class DialogueManager:
    def __init__(self):
        ...
        #ai提炼的一个限制
        self.limit_tokens = self.max_tokens*0.75  # 设置一个安全的限制，避免超过模型的最大上下文窗口
        ...
        self.have_tokens = self.limit_tokens - self.history_tokens #提炼记忆的token数
        self.del_token = self.del_memory_tokens - self.history_tokens #删除记忆的token数
        print(f"还剩 {self.del_token} 个 token 可以使用。")

    def steamChat(self, message):
        ...
        for chunk in stream:
            ...
            # 如果块中包含 usage 信息（通常最后一个块 choices 为空但 usage 存在）
        ...
        if match:
            ...
            self.prompt,respon = get.returnAiResponse()
        ...
        # self.new_tokens = completion_tokens
        ...
        if self.have_tokens <= 0:
            ...
            cleaner = RefineMemory(new_prompt, store=store, min_chars=3)
            memories = cleaner.getFromOpenAI(self.key, self.model, self.base_url, dialog_ids=dialog_ids)
            RefineMemory.sendPromptToAi()
    ...
    def run(self):
        while True:
            ...
            if user_input.lower() == "exit":
                print("退出对话。")
                break
            self.steamChat(user_input)


def main():
    manager = DialogueManager()
    # manager.steamChat("你好")
    manager.run()
